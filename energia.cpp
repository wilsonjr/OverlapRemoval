...
void create_graph(vector<vertice>& grafo, vector<double> pontos, int k)
{
    for( int i = 0; i < grafo.size(); i += 2 ) {
        set<point> elems;

        for( int j = i+2; j < grafo.size(); j += 2 ) {
            point p(j, distancia(pontos[i], pontos[i+1], pontos[j], pontos[j+1]));

            if( elems.size() >= k && elems.rbegin()->dist > p.dist )
                elems.erase(*elems.rbegin());

            elems.insert(p);
        }

        set<point>::iterator it = elems.begin();
        for( ; it != elems.end(); ++it ) {
            grafo[i].adj.push_back(it->id);
            grafo[it->id].adj.push_back(i);
        }
    }
}
